...
supportRouter.post('/messages', requireAuth, async (request, response, next) => {
  try {
    ...
    const { data: latestInquiry, error: inquiryLookupError } = await adminSupabase
      .from('support_inquiries')
      .select('id,status')
      .eq('user_id', request.userId)
      .order('updated_at', { ascending: false })
    ...
    if (latestInquiry?.status === 'closed') return response.status(409).json({ error: '관리자가 채팅을 종료했습니다.' });
    let inquiryId = latestInquiry?.id;
    if (!inquiryId) {
      const { data: inquiry, error: inquiryError } = await adminSupabase
        .from('support_inquiries')
